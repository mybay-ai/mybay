...
export class CodexRunPreparationProvider implements RuntimeRunPreparationProvider {
  public createController(dependencies: RuntimeRunPreparationDependencies): RuntimeRunPreparationController {
    const createSessionBinding: RuntimeRunPreparationController["createSessionBinding"] = async (
      ...
      const response = await dependencies.request({
        ...
        method: "POST",
        path: "/api/sessions",
        body: { title: title || "MyBay Codex Conversation" },
        timeoutMs: 10000,
      });
      const sessionId = response.ok ? extractSessionId(response.json) : null;
      if (!sessionId) {
        const error = new Error("CODEX_SESSION_CREATE_FAILED");
    ...
    return {
      ...
      // Codex App Server resumes its persisted native thread.
      ...
      buildRunPayload: (options) => {
        ...
        return {
          ...
          model_options: { reasoning_effort: options.reasoningEffort === "fast" ? "low" : options.reasoningEffort === "deep" ? "high" : "medium" },
        };
      },
    };
  }
...
export const codexRunPreparationProvider = Object.freeze(new CodexRunPreparationProvider());
