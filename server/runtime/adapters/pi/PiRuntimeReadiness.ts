...
export async function probePiRuntimeReadiness(instance: any) {
  ...
  if (!keyResolution.ok || !keyResolution.apiKey) {
    return {
      ...
      gateway_error: keyResolution.error || "PI_BRIDGE_API_KEY_MISSING",
      gateway_services: {},
  ...
  const ready = Boolean(response.ok
    && response.json?.runtime === "pi"
    && response.json?.features?.run_submission === true
    && response.json?.features?.run_status === true);
  return {
    gateway_ready: ready,
    gateway_status: ready ? "running" : "unhealthy",
    gateway_error: ready ? null : (response.error || `PI_CAPABILITY_HTTP_${response.statusCode || 0}`),
    gateway_services: ready ? { pi_rpc_bridge: "running" } : {},
    checked_at: checkedAt,
    configured_channels: 0,
    connected_channels: 0,
    channel_status: {},
