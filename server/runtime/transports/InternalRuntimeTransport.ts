...
export function sanitizeRuntimeIdempotencyKey(value: unknown): string | undefined {
  const key = normalizeHeaderValue(value);
  if (!key || key.length > 256) return undefined;
  return /^[A-Za-z0-9._:-]+$/.test(key) ? key : undefined;
}

export function sanitizeRuntimeRequestHeaders(
  headers?: Record<string, string>,
): Record<string, string> | undefined {
  const safeHeaders: Record<string, string> = {};
  const idempotencyKey = sanitizeRuntimeIdempotencyKey(
...
export async function requestInternalRuntimeAPI(
  options: RuntimeRequestOptions,
): Promise<RuntimeRequestResult> {
  const { instanceId, method, path, body, timeoutMs = 15000, headers, sessionId } = options;
  try {
    const instance = await dbAdapter.getInstanceById(instanceId);
    if (!instance) return { ok: false, statusCode: 404, error: "INSTANCE_NOT_FOUND" };
    const keyResolution = resolveInstanceInternalApiKey(instance);
    if (!keyResolution.ok || !keyResolution.apiKey) {
      return { ok: false, statusCode: 400, error: keyResolution.error || "RUNTIME_INTERNAL_API_KEY_MISSING" };
    }
    const response = await requestTraefikInternal({
      ...
      body,
      timeoutMs,
      headers: sanitizeRuntimeRequestHeaders(headers),
    ...
    return { ok: false, statusCode: 500, error: error.message || "INTERNAL_RUNTIME_REQUEST_ERROR" };
  }
}

export async function streamInternalRuntimeEventsAPI(
  instanceId: string,
