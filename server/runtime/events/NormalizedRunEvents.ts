...
export interface NormalizedRunEventPolicy {
  /** Adapter-only recovery for a failed event before any text was delivered. */
  readonly shouldReconcileEmptyFailure?: (error: unknown) => boolean;
}

/** Interprets the MyBay Runs event vocabulary, not an arbitrary native protocol. */
export class NormalizedRunEventProvider implements RuntimeRunEventProvider {
  private readonly policy: Readonly<NormalizedRunEventPolicy>;

  constructor(policy: NormalizedRunEventPolicy = {}) {
  ...
  createController(dependencies: RuntimeRunEventDependencies): RuntimeRunEventController {
    ...
    async function completeTerminalEvent(
      ...
      if (eventType === "run.failed" || eventType === "run.error") {
        ...
        if (!tracker?.lastPartialOutput && shouldReconcileEmptyFailure?.(upstreamError)) {
...
export const normalizedRunEventProvider = Object.freeze(new NormalizedRunEventProvider());
