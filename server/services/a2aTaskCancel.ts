...
export async function cancelA2ATask(link: A2ATaskLink, send: (taskId: string) => Promise<any>) {
  ...
  return updateA2ATaskLink(link.id, { task, remoteState: task.status.state, state: 'finished', lookupState: undefined, checkedAt, cancelState: 'confirmed', cancelCheckedAt: checkedAt });
}
