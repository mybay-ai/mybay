...
export interface RunEventTracker {
  lastPartialOutput: string;
  sentSteps: Map<string, string>;
  activeToolIds: Map<string, string[]>;
}

interface RunEventTarget {
  id: string;
  partial_output?: unknown;
}

interface RunHermesEventInterpreterDependencies {
  addEvent(runId: string, event: string, data: string, ownerId?: string): void;
  completeTerminal(run: RunEventTarget, event: Record<string, any>, upstreamRunId: string): Promise<boolean>;
...
export interface RunHermesEventInterpreter {
  get(runId: string): RunEventTracker | undefined;
  getOrCreate(runId: string, initialPartialOutput?: unknown): RunEventTracker;
  clear(runId: string): void;
  emitStep(runId: string, tracker: RunEventTracker, step: unknown, ownerId?: string): void;
  handle(run: RunEventTarget, event: unknown, upstreamRunId?: string): void;
}
...
export function createRunHermesEventInterpreter(
  dependencies: RunHermesEventInterpreterDependencies
): RunHermesEventInterpreter {
  const trackers = new Map<string, RunEventTracker>();

  function getOrCreate(runId: string, initialPartialOutput: unknown = ""): RunEventTracker {
  ...
  function emitStep(runId: string, tracker: RunEventTracker, step: unknown, ownerId?: string): void {
  ...
  function scheduleTerminal(
    ...
    void dependencies.completeTerminal(run, event, upstreamRunId).catch((error) => {
      dependencies.warn(
  ...
  function handle(run: RunEventTarget, rawEvent: unknown, upstreamRunId?: string): void {
