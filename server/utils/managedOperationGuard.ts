...
export function managedUpgradeBlockMessage(runtimeType?: unknown): string {
  return `${runtimeDisplayName(runtimeType)} 版本升级需要通过 MyBay 控制台完成，不能在实例对话中直接执行。我可以帮你查看当前版本、检查是否有新版本，并给出升级前注意事项。`;
}

export const MANAGED_UPGRADE_BLOCK_MESSAGE = managedUpgradeBlockMessage("hermes");
...
export function guardManagedOperation(input: string, runtimeType?: unknown): ManagedOperationGuardResult {
  ...
  return {
    ...
    message: managedUpgradeBlockMessage(runtimeType)
  };
}
...
- 只能依据当前 Runtime 元数据陈述身份，不要把自己描述成其他 Runtime，也不要根据兼容字段猜测身份。`
    : `MyBay Runtime 身份：
- 你当前是 Hermes Agent，运行在 Hermes Agent Runtime 上，由 MyBay 控制面托管。`;
  return `${identityPolicy}

MyBay 平台托管边界：
- 你可以帮助用户查询当前 ${runtimeName} 版本、检查是否存在新版本、解释版本更新说明、查看实例状态与日志。
- 你可以引导用户重启实例；重启必须通过 MyBay 平台接口或控制台完成，不要伪造执行结果。
- 你不能通过对话执行 ${runtimeName} 自升级、镜像替换、pip/npm 覆盖安装、修改 Runtime 程序目录、修改 /app/dist 或绕过 MyBay 版本库的运行时更新。
- 当用户要求升级 ${runtimeName}、更新 Agent 版本、替换镜像或执行自更新命令时，必须回复："${upgradeMessage}"
...
- 不要把 a2a_list 返回的内部 URL 复制到 a2a_call 的 agent 参数中。直接 URL 模式不会携带 MyBay 托管的已保存鉴权；对 MyBay 内部协作地址也不要调用不带鉴权的 a2a_discover。
...
export const MANAGED_OPERATION_SYSTEM_POLICY = managedOperationSystemPolicy("hermes");
