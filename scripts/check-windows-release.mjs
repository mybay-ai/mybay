...
const envExample = readAsText(".env.example");
...
const compose = readAsText("docker-compose.windows.yml");
...
const uninstall = readAsText("scripts/windows-control.ps1");
...
const startLauncher = readAsText("Start-MyBay.bat");
...
const preflight = readAsText("scripts/windows-preflight.ps1");
...
const acceptance = readAsText("scripts/windows-acceptance.ps1");
...
if (!readAsText("Collect-Diagnostics.bat").includes("windows-acceptance.ps1")) {
  throw new Error("Windows diagnostics launcher must call the acceptance evidence collector.");
}

console.log(`[Release] Windows archive verified: ${archivePath} (${names.length} files, v${packageJson.version}).`);
