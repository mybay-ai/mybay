...
export function AgentDeploymentHome({ instances, onTabChange }: AgentDeploymentHomeProps) {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 lg:grid-cols-[1.35fr_0.65fr]"><Card className="p-5 sm:p-6"><div className="mb-5 flex items-center justify-between gap-3"><div><h3 className="font-bold text-content">{zh ? "部署进度" : "Deployment journey"}</h3><p className="mt-1 text-xs text-content-muted">{zh ? "每个完成状态均来自真实检测或验收记录。" : "Every completed state comes from a live check or acceptance record."}</p></div><span className="flex items-center gap-1 text-xs font-semibold text-indigo-600">{journeyLoading && <Loader2 className="h-3.5 w-3.5 animate-spin" />}{journey?.completed || 0}/{journey?.total || 4}</span></div><div className="space-y-2">{steps.map((step, index) => <button type="button" key={step.key} onClick={step.action} className="group flex w-full items-start gap-3 rounded-xl border border-outline p-3 text-left transition hover:border-indigo-200 hover:bg-indigo-50/30"><div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${step.done ? "bg-emerald-100 text-emerald-700" : step.status === "attention" ? "bg-amber-100 text-amber-700" : "bg-control-hover text-content-muted"}`}>{step.done ? <CheckCircle2 className="h-4 w-4" /> : step.status === "attention" ? <TriangleAlert className="h-4 w-4" /> : <Circle className="h-4 w-4" />}</div><div className="min-w-0 flex-1"><div className="text-sm font-semibold text-content">{index + 1}. {step.title}</div><div className="mt-0.5 text-xs leading-5 text-content-muted">{step.description}</div></div><ArrowRight className="mt-1 h-4 w-4 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-indigo-500" /></button>)}</div></Card><div className="space-y-4"><QuickAction icon={KeyRound} title={zh ? "模型凭据" : "Model credentials"} description={zh ? "添加或轮换 BYOK API Key" : "Add or rotate BYOK API keys"} onClick={() => onTabChange("credentials")} /><QuickAction icon={MessageSquare} title={zh ? "消息渠道" : "Messaging channels"} description={zh ? "配置、连接并完成真实消息验收" : "Configure, connect, and accept real messaging"} onClick={() => navigate(hasInstances ? APP_ROUTES.INSTANCES : APP_ROUTES.DEPLOY)} /><QuickAction icon={Server} title={zh ? "实例运行状态" : "Instance runtime"} description={zh ? "查看部署、日志、诊断与恢复" : "Review deployment, logs, diagnostics, and recovery"} onClick={() => navigate(APP_ROUTES.INSTANCES)} /></div></section>
  </div>;
}
...
function QuickAction({ icon: Icon, title, description, onClick }: { icon: any; title: string; description: string; onClick: () => void }) { return <button type="button" onClick={onClick} className="group flex w-full items-center gap-3 rounded-xl border border-outline bg-surface p-4 text-left shadow-sm transition hover:border-indigo-200 hover:shadow"><div className="rounded-lg bg-indigo-50 p-2 text-indigo-600"><Icon className="h-5 w-5" /></div><div className="min-w-0 flex-1"><div className="text-sm font-bold text-content">{title}</div><div className="mt-0.5 text-xs leading-5 text-content-muted">{description}</div></div><ArrowRight className="h-4 w-4 text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-indigo-500" /></button>; }
