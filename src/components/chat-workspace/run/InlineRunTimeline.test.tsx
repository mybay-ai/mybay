...
describe("interleaved run message rendering", () => {
  it("renders each narration once around its tool and keeps unsafe metadata out of the DOM", async () => {
    ...
    const execution = reduceRunEvents(createRunExecutionState({ runId: "r", conversationId: "c" }), [
      ...
      { seq: 2, runId: "r", type: "commentary.added", payload: { id: "note", text: "RepositoryChecked." } },
      { seq: 3, runId: "r", type: "tool.started", payload: { id: "t", tool: "file", metadata: { file_path: "safe.html", command: "DO_NOT_RENDER" } } },
      { seq: 4, runId: "r", type: "tool.completed", payload: { id: "t", tool: "file" } },
      { seq: 5, runId: "r", type: "text.delta", payload: { delta: "FinalResponse." } },
    ]);
    ...
    expect(html.split("RepositoryChecked.")).toHaveLength(2);
    expect(html.split("FinalResponse.")).toHaveLength(2);
    expect(html.indexOf("NarrationBefore.")).toBeLessThan(html.indexOf("safe.html"));
    expect(html.indexOf("RepositoryChecked.")).toBeLessThan(html.indexOf("safe.html"));
