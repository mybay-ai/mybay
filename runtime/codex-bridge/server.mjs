...
import { CodexQuestionBridge, questionBridgeConfig } from "./question-bridge.mjs";
...
export const CODEX_BRIDGE_FEATURES = Object.freeze({
  ...
  structured_questions: true, a2a_tools: false, managed_collaboration: false,
  session_context_usage: true, automatic_run_cleanup: true, manual_compaction: false, session_resources: false,
});
...
export async function startServer(options = {}) {
  ...
  const bridgeConfig = questionBridgeConfig(env);
  const runtime = new CodexRuntime({ rpc, dataDir, workspace, model: env.CODEX_MODEL || undefined,
    externalSandbox: env.CODEX_EXTERNAL_SANDBOX === "true",
    a2aPeers: readConfiguredPeers(env.MYBAY_A2A_PEERS_JSON || ""),
    questionBridge: bridgeConfig ? new CodexQuestionBridge(bridgeConfig) : null });
  ...
  async function handle(request, response) {
    ...
    if (request.method === "GET" && url.pathname === "/v1/capabilities") {
      ...
      return json(response, 200, { runtime: "codex", auth_ready: ready, features: { ...CODEX_BRIDGE_FEATURES, run_submission: ready,
        a2a_tools: a2aReady, managed_collaboration: a2aReady, structured_questions: Boolean(bridgeConfig) },
