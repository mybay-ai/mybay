...
- 从目标发布版本建立独立分支或工作区；调整开发目录前先检查 `git status` 和 `git worktree list`。
- 临时验收日志和草稿统一放在 `tmp/`，发行包放在 `release/`，构建产物放在 `dist/`。这些目录不会提交到 Git。
- `data/` 保存运行数据，`backups/` 保存备份，不应作为源码整理的一部分删除。
- 清理历史工作区前，先核对未提交修改和使用该路径的本地服务。依赖备份目录（`node_modules*`）必须排除在 Git 和 Docker 构建上下文之外。
- 保留认证证据及其引用的产物；发布检查依赖这些文件。已跟踪的目录清单和生成文档使用现有 npm 脚本更新。
- 按职责拆分模块，测试随模块保留；分批移动脚本或组件，并同步修改构建、CI 和文档引用。
