...
Codex now has an isolated App Server bridge pinned to official CLI 0.153.4, a registered Driver, account-import deployment and capability-based readiness. Deployment is available by default without an opt-in flag; certification remains Experimental. See [bridge boundaries](../runtime/codex-bridge/README.md). Claude Code is not implemented.
