...
## Codex 0.154.0 upgrade and rollback closure

This section supersedes the earlier Codex upgrade and rollback `NOT_RUN` boundary. The release catalog now admits native Codex 0.154.0 as the latest Experimental image and retains 0.153.4 as its supported local rollback point. Upgrade selection rejects unknown images, the product preflight checks the cached image and host architecture, and the version repository exposes upgrade, rollback and audit-log actions for Codex instances.

The isolated restored product on port 4362 completed `0.153.4 -> 0.154.0`, continued its restored conversation on native thread `01a088fa-03b9-7823-aaf0-6132d63bfd52`, rolled back to 0.153.4, continued the same thread again, and completed a final upgrade to 0.154.0. The final preflight reported 9 passed checks, 2 warnings and 0 blockers. The final container is healthy, chat-ready, and has exactly one authoritative `PORT=9119` value. A live 0.154.0 task entered `running`, accepted a stop request and ended `cancelled` with `CANCELLED_UPSTREAM`.

Three failures were retained rather than hidden: Hermes-specific readiness initially rejected Codex, a recreated container left a stale target in the Runtime cache, and the upgrade path created duplicate `PORT` values. Runtime-aware readiness, target invalidation and environment normalization correct those failures; focused regression tests cover each boundary.

The browser version repository shows both releases, current 0.154.0 state, rollback point 0.153.4, disabled latest-version action and enabled rollback action. Sanitized evidence is retained in `codex-upgrade-rollback-20260910.json`. This closes same-host Windows Docker Desktop upgrade and rollback for the Experimental Codex Runtime. Cross-platform upgrade, Windows-native-to-Linux restore and production publication remain `NOT_RUN`. The main localhost:3000 service, Hermes instances and Feishu connections were not changed.
